...
Процедура ПриСозданииНаСервере(Форма, Знач ДополнительныеПараметры = Неопределено) Экспорт
	...
	ДобавитьРеквизитФормыНастройкиДинамическихСписков(Форма);
...
Процедура ПередЗагрузкойПользовательскихНастроекНаСервере(
	...
	Если ЭтоОткрытиеФормы(ОбщиеНастройки) Тогда
		...
		Иначе
			...
			УстановитьПериодСпискаПриОткрытии(Форма, НастройкиСписка, Элемент);
...
Процедура ПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ОбработатьСобытиеОбновленияДинамическогоСписка(Настройки);
...
Процедура ПрименитьНастройкиПодсистемыКСписку(Форма, ОбщиеНастройки, НастройкиСписка)
	...
	ИТМ_АдминистрированиеДинамическихСписковПереопределяемый.ПередПрименениемНастройкиПодсистемыКСписку(
		Форма, ОбщиеНастройки, НастройкиСписка, СтандартнаяОбработка);	
	...
	ПроверитьЗапретыОткрытияСписка(ОбщиеНастройки, НастройкиСписка, Список);
	Если ОбщиеНастройки.Отказ Тогда
		Возврат;
	КонецЕсли;
	
	ПрименитьНастройкиИсточникаПодсистемыКСписку(НастройкиСписка, Список);
	
	ПрименитьОсновныеНастройкиПодсистемыКСписку(НастройкиСписка, Список);
			
	Если НастройкиСписка.ПараметрыОтладки.РежимОткрытияНастроек Тогда
		
		ПрименитьРежимОткрытияНастроекКСписку(НастройкиСписка, Список);
		
	Иначе
		
		ПрименитьНастройкиОтборовПодсистемыКСписку(НастройкиСписка, Список);
	
		ПрименитьНастройкиСортировкиПодсистемыКСписку(НастройкиСписка, Список);
	
		ПрименитьНастройкиГруппировкиПодсистемыКСписку(НастройкиСписка, Список);
	
		ПрименитьНастройкиУсловногоОформленияПодсистемыКСписку(НастройкиСписка, Список);
		
	КонецЕсли;
		
	ПрименитьНастройкиИсторииОбновленияПодсистемыКСписку(Форма, НастройкиСписка, Список);
	
	ИТМ_АдминистрированиеДинамическихСписковПереопределяемый.ПослеПримененияНастройкиПодсистемыКСписку(
		Форма, ОбщиеНастройки, НастройкиСписка);
...
Процедура ПрименитьРежимОткрытияНастроекКСписку(НастройкиСписка, Список)
...
//  НастройкиСписка - см. ИТМ_АдминистрированиеДинамическихСписковКлиентСервер.ШаблонПараметровДинамическогоСписка
//  Список - ДинамическийСписок - Список
Процедура ПрименитьНастройкиИсточникаПодсистемыКСписку(НастройкиСписка, Список)
...
//  НастройкиСписка - см. ИТМ_АдминистрированиеДинамическихСписковКлиентСервер.ШаблонПараметровДинамическогоСписка
//  Список - ДинамическийСписок - Список
Процедура ПрименитьОсновныеНастройкиПодсистемыКСписку(НастройкиСписка, Список)
...
//  НастройкиСписка - см. ИТМ_АдминистрированиеДинамическихСписковКлиентСервер.ШаблонПараметровДинамическогоСписка
//  Список - ДинамическийСписок - Список
Процедура ПрименитьНастройкиОтборовПодсистемыКСписку(НастройкиСписка, Список)
	...
	//@skip-check bsl-legacy-check-dynamic-feature-access - подсистема использует точечную блокировку подчиненных полей
...
//  НастройкиСписка - см. ИТМ_АдминистрированиеДинамическихСписковКлиентСервер.ШаблонПараметровДинамическогоСписка
//  Список - ДинамическийСписок - Список
Процедура ПрименитьНастройкиСортировкиПодсистемыКСписку(НастройкиСписка, Список)
...
//  НастройкиСписка - см. ИТМ_АдминистрированиеДинамическихСписковКлиентСервер.ШаблонПараметровДинамическогоСписка
//  Список - ДинамическийСписок - Список
Процедура ПрименитьНастройкиГруппировкиПодсистемыКСписку(НастройкиСписка, Список)
...
//  НастройкиСписка - см. ИТМ_АдминистрированиеДинамическихСписковКлиентСервер.ШаблонПараметровДинамическогоСписка
//  Список - ДинамическийСписок - Список
Процедура ПрименитьНастройкиУсловногоОформленияПодсистемыКСписку(НастройкиСписка, Список)
...
//  НастройкиСписка - см. ИТМ_АдминистрированиеДинамическихСписковКлиентСервер.ШаблонПараметровДинамическогоСписка
//  Список - ДинамическийСписок - Список
Процедура ПрименитьНастройкиИсторииОбновленияПодсистемыКСписку(Форма, НастройкиСписка, Список)
...
//  НастройкиСписка - см. ИТМ_АдминистрированиеДинамическихСписковКлиентСервер.ШаблонПараметровДинамическогоСписка
//  Список - ДинамическийСписок - Список
Процедура ПроверитьЗапретыОткрытияСписка(ОбщиеНастройки, НастройкиСписка, Список)
...
Процедура ОбработатьСобытиеОбновленияДинамическогоСписка(Настройки)
...
Функция ЗапрещенныеПоляСписка(НастройкиОграничений, Список, ДоступныеПоля)
	...
	Для Каждого ДоступноеПоле Из ДоступныеПоля.Элементы Цикл
		...
		ОписаниеПоля = ЗапрещенныеПоляСписка_ОписаниеПоля(ДоступноеПоле, Список);
...
Функция ЗапрещенныеПоляСписка_ОписаниеПоля(ДоступноеПоле, Список)
...
Процедура УстановитьПериодСпискаПриОткрытии(Форма, НастройкиСписка, Элемент)
...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма
Процедура ДобавитьРеквизитФормыНастройкиДинамическихСписков(Форма)
...
Процедура ЗаполнитьНастройкиДинамическихСписковНаФорме(Форма, ОбщиеНастройки)
	...
	Для Каждого ИмяЭлементаДС Из ИменаЭлементовДС Цикл
		...
		НастройкиДС.ИмяЭлемента = ИмяЭлементаДС;
		НастройкиДС.ИмяРеквизита = ЭлементФормы.ПутьКДанным;
		НастройкиДС.ПараметрыОтладки = ИТМ_АдминистрированиеДинамическихСписковКлиентСервер.ШаблонПараметровОтладки();
